...
export async function getRecommendedUsers(req, res) {
  try {
    ...
    const currentUser = await User.findById(currentUserId); // or const currentUser = req.user
...
export async function getMyFriends(req, res) {
  try {
    const currentUserFriends = await User.findById(req.user.id)
      .select("friends")
      .populate("friends", "fullName profilePic location");

    res.status(200).json(currentUserFriends.friends);
  } catch (error) {
...
export async function sendFriendRequest(req, res) {
  try {
    ...
    //chacking if users are already connected!!
    if (recipient.friends.includes(myId)) {
      return res
        .status(400)
        .json({ message: "You are already friends with this user" });
    }
    //checking if request already exists or not!!
...
export async function acceptFriendRequest(req, res) {
  try {
    ...
    //checking the current user is the recipient
