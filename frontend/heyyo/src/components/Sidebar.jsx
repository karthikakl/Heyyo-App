...
const Sidebar = ({selectedUser,setSelectedUser}) => {
  ...
  return (
    ...
    text-white ${selectedUser ? 'max-md:hidden':''}`}>
      <div className='pb-5'>
          <div className='flex justify-between items-center'>
            ...
            <div className='relative py-2 group'>
              <img src={assets.menuIcon} alt='menu' className='max-h-8
              cursor-pointer'/>
     ...
     <div className='bg-[#f9f8f7] rounded-full flex items-center gap-2 py-3 px-4 
     mt-5'>
      <img src={assets.searchIcon} alt="searchIcon" className='w-4' />
      <input type="text" className='bg-transparent border-none outline-none 
      text-black text-xs placeholder-[#0e0e0e] flex-1' placeholder='Search User....' />
     </div>
      ...
      <div className='flex flex-col'>
        {userDummyData.map((user,index)=>(
          ...
          cursor-pointer max-sm:text-sm ${selectedUser ?._id===user._id && 'bg-gray-500'}`}>
            <img src={user?.profilepic || assets.avatar} alt='pic'
            ...
            flex justify-center items-center rounded-full bg-red-700'>{index}</p>}
          </div>
        ))}
